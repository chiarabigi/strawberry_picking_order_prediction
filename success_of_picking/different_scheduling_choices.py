...
def unite_infos(json_annotations_path):
    ...
    json_annotations_path = base_path + '/choices_test/{}/raw/gnnann.json'.format(policy)
...
class Choices(torch.nn.Module):
    ...
    def forward(self):
        ...
        json_annotations_path = base_path.strip('success_of_picking') + '/dataset/data_test'
        json_annotations2_path = unite_infos(json_annotations_path)  # adds target information
        graph_data_picking_success = cfg_p.DATASET(base_path + '/choices_test/{}'.format(policy))
