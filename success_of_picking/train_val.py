...
import config_sop as cfg
from utils.metrics import get_occlusion1
...
def train_one_epoch():
    ...
    for i, batch in enumerate(train_loader, 0):
        ...
        batch.to(device)
        outputs = model(batch)
        weights = torch.ones_like(batch.y) / 0.3 + (1.0 - 1.0 / 0.3) * batch.y
...
def calculate_metrics(y_pred, y_true):
    ...
    disp.plot()
    if BestModel:
        plt.savefig(os.path.join('./plots/plots_sop', 'CM{}_{}_{}_{}_L2{}_{}.jpg'.format(Phase, hiddenLayers, cfg.NL, batchSize, weightDecay, SeedNum)))
...
def draw_curve(current_epoch, cfg):
    ...
    if current_epoch == 0:
        ...
        ax1.text(0.5, 0.2, 'V' + str(best_vaccuracy),
                 horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
    fig.savefig(os.path.join('./plots/plots_sop', 'train_{}_{}_{}_L2{}_{}.jpg'.format(cfg.HL, cfg.NL, cfg.BATCHSIZE, cfg.WEIGHTDECAY, cfg.SEEDNUM)))
...
for epoch in trange(1, NumEpochs + 1):
    if early_stopping_counter <= 10:
        ...
        if val_loss < best_vloss:
            best_vloss = val_loss
            model_path = 'best_models/model_sop_{}'.format(timestamp)
